...
METHOD New( nPort, oPP ) CLASS HbSocket2
...
METHOD KillClient( oClient ) CLASS HbSocket2
...
METHOD OnAccept() CLASS HbSocket2
   ...
   do while ! ::lExit
      ...
      if ! Empty( pClientSocket := HB_SocketAccept( ::pSocket,, 500 ) )
         ...
         hb_mutexLock( ::hMutexUser )
         ::NewId()
         oClient = HBSocket2Client():New( Self )
...
METHOD OnRead( oClient ) CLASS HbSocket2
...
METHOD SendData( oClient, cSend ) CLASS HbSocket2
...
METHOD New( oSrv ) CLASS  HbSocket2Client
...
METHOD End() CLASS HbSocket2Client 
...
METHOD TimerConnection( nTimeOut ) CLASS HbSocket2Client
...
METHOD TimeOut() CLASS HbSocket2Client
...
FUNCTION LogFile2( cFileName, aInfo )
...
FUNCTION UErrorHandler( oErr, oServer )
   ...
   if oServer:cErrorLog == NIL
         oServer:cErrorLog := "error.log"
   endif
   LogFile2( oServer:cErrorLog, { GetErrorDesc( oErr ) } )
